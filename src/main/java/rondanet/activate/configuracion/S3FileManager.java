...
import rondanet.activate.ActivateConfiguration;
import rondanet.activate.configuracion.interfaces.IS3FileManager;
...
public class S3FileManager implements IS3FileManager {
	...
	private ActivateConfiguration configuration;

	public S3FileManager(ActivateConfiguration configuration/* , ParamsDAO paramsDAO */) {
	...
	public AmazonS3 crearConexion() {
		return AmazonS3ClientBuilder.standard().withRegion(obtenerRegion())
				.withCredentials(new AWSStaticCredentialsProvider(this.credentials)).build();
	}

	String obtenerRegion() {
		String region = "us-east-2";
		Param param = null;// = paramsDAO.findByNombre(Propiedades.AWS_REGION);
		if (param != null) {
